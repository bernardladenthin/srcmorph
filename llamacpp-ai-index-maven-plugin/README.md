...
> for the product overview, badges, and build status. **This plugin's Maven coordinates, package,
> and property names are unchanged** (`net.ladenthin:llamacpp-ai-index-maven-plugin`, goal prefix
> `ai-index`, properties `aiIndex.*`); a future release will relocate it to
> `net.ladenthin:srcmorph-maven-plugin`, but that has not happened yet. Internally, the plugin now
> depends on [`net.ladenthin:srcmorph`](../srcmorph/README.md) (a sibling reactor module) for all of
