 ...
 * {@link PackageIndexer}.
 *
 * <p>Iterates over a list of {@link net.ladenthin.srcmorph.config.AiFieldGenerationConfig} entries, prepares the
 ...
 * configured maximum input character budget.</p>
 *
...
public class AiFieldGenerationSupport {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(AiFieldGenerationSupport.class);
     ...
     *
     * <p>For each non-null {@link net.ladenthin.srcmorph.config.AiFieldGenerationConfig}:</p>
     * <ol>
     *   <li>The prompt is prepared via {@link net.ladenthin.srcmorph.prompt.AiPromptPreparationSupport#preparePrompt}.</li>
     *   <li>A trim warning is logged if the source was truncated and
     *       {@link net.ladenthin.srcmorph.config.AiGenerationConfig#isWarnOnTrim()} is {@code true}.</li>
        ...
        for (AiFieldGenerationConfig fieldGeneration : fieldGenerations) {
            ...
            if (preparedPrompt.trimmed()) {
                ...
                if (oversize == AiOversizeStrategy.DETERMINISTIC) {
                    ...
                    LOGGER.info(
                            "Oversize {} '{}' -> deterministic body (no model), {} chars",
                            contextType,
                            contextFile,
                            sourceText.length());
                    continue;
            ...
            if (preparedPrompt.trimmed() && generationConfig.isWarnOnTrim()) {
                LOGGER.warn(
                        "Trimmed AI input for {}" + TRIM_WARN_FIELD_LABEL
                                + "body': {} (source chars {} -> {}, available source chars {}, max input chars {})",
                        contextType,
                        contextFile,
                        preparedPrompt.originalSourceLength(),
                        preparedPrompt.trimmedSourceLength(),
                        preparedPrompt.availableSourceChars(),
            ...
            LOGGER.info(
                    PROCESSING_LOG_PREFIX + "{} '{}'" + PROCESSING_LOG_SIZE_INFIX + "{}" + PROCESSING_LOG_TOKENS_INFIX
                            + "{}" + PROCESSING_LOG_ETA_INFIX + "{}" + PROCESSING_LOG_DISCLAIMER,
                    contextType,
                    contextFile,
                    sourceSizeKb,
                    timeEstimator.estimatePromptTokens(processedSourceChars),
                    timeEstimator.formatDuration(estimatedSeconds));

            LOGGER.info(
                    "Generating field '{}' with temperature={}, maxInputChars={}",
            ...
            LOGGER.info(
                    GENERATED_LOG_PREFIX + "{} '{}" + GENERATED_LOG_IN_INFIX + "{}" + GENERATED_LOG_ACTUAL_INFIX + "{}"
                            + GENERATED_LOG_SUFFIX,
                    contextType,
                    contextFile,
                    timeEstimator.formatDuration(actualSeconds),
                    timeEstimator.formatDuration(estimatedSeconds));

            if (compatibilityHelper.isBlank(generated)) {
                LOGGER.warn(
                        EMPTY_OUTPUT_WARN_PREFIX + "{}" + TRIM_WARN_FIELD_LABEL + "{}': {}",
                        contextType,
                        fieldGeneration.getPromptKey(),
                        contextFile);
            }
    ...
    private String mapReduceSummarize(
        ...
        LOGGER.info(
                "Oversize file '{}' -> mapReduce: {} chars in {} chunk(s)",
                contextFile,
                sourceText.length(),
                chunks.size());
        ...
        for (final String chunk : chunks) {
            ...
            partials.add(summarizeChunk(fieldGeneration.getPromptKey(), contextFile, chunk, baseHeader, safeWindow)
                    .trim());
            LOGGER.info("  mapReduce chunk {}/{} summarized ({} chars)", index, chunks.size(), chunk.length());
        }
    ...
    private String reducePartials(
        ...
        while (level.size() > 1) {
            ...
            LOGGER.info("  mapReduce reduce round {}: {} -> {} partial(s)", round, level.size(), next.size());
    ...
    private String generateWithOverflowRetry(
        ...
        for (int attempt = 0; ; attempt++) {
            ...
            try {
                ...
                LOGGER.warn(
                        "AI provider rejected the prompt (context overflow) for {}; retrying with a smaller window"
                                + " ({} -> {} chars)",
                        contextFile,
                        budget,
                        reduced);
     ...
     *
     * <p>When the {@link net.ladenthin.srcmorph.config.AiGenerationConfig#getCharsPerToken()} value is greater than zero,
    ...
    private int calculateAndLogMaxInputChars(final AiGenerationConfig config, final int basePromptLength) {
        ...
        LOGGER.info(
                "Maximum input characters for source code before trimming. Calculated as: (context_size x {}) -"
                        + " overhead",
                charsPerToken);
        LOGGER.info("  Context: {} tokens", contextSize);
        LOGGER.info("  Chars per token: ~{}", charsPerToken);
        LOGGER.info("  [Total available]: {} x {} = {} chars", contextSize, charsPerToken, totalChars);
        LOGGER.info("  Overhead (conservative estimate):");
        LOGGER.info("  - Prompt template: ~{} chars", promptChars);
        LOGGER.info("  - EOF marker: ~{} chars", eofChars);
        LOGGER.info("  - Max output ({} tokens x {}): ~{} chars", maxOutputTokens, charsPerToken, outputChars);
        LOGGER.info("  - Safety margin: ~{} chars", safetyChars);
        LOGGER.info("  - [Subtotal overhead]: ~{} chars", overheadTotal);
        LOGGER.info("  Available for source: [Total available] - [Subtotal overhead]");
        LOGGER.info("  Available for source: {} - {} = {} chars", totalChars, overheadTotal, availableChars);
