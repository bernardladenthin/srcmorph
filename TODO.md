...
- **Calibrate uses the engine's exact per-call timings (no open limitation).** `ai-index:calibrate` reads the model's own `prompt_per_second` / `predicted_per_second` (and `prompt_n` / `predicted_n`) from the chat response: `LlamaCppJniAiGenerationProvider.generateWithTimings` calls `chatComplete(...)` and parses the OAI JSON via the binding's `ChatResponseParser` (the plain `chatCompleteText` had discarded the `timings`). The wall-clock + `charsPerToken` estimate remains only as a fallback for the mock provider / a build that returns no timings.
