...
- **Mutation-testing threshold enforcement (PIT)** — `pom.xml` wires `<mutationThreshold>100</mutationThreshold>`. **Scope expanded 2026-06-07** from the single `AiCompletionParser` target to an explicit **19-class** list (config / document / prompt / provider / support value + support classes; 138 mutations, all killed). Still open (optional): `document.AiMdDocumentCodec` / `AiMdHeaderCodec` / `prompt.AiPromptPreparationSupport` (need careful codec fixtures). `support.AiPathSupport` and `provider.AiGenerationProviderFactory` are excluded — equivalent / native-dependent mutants (see crossrepostatus "Deliberate non-parity").
