...
- **Null-safety refinement.** JSpecify + NullAway are now enforced at compile time (Error Prone compiler plugin in `pom.xml`, `@NullMarked` on the package via `package-info.java`, JDK module exports in `.mvn/jvm.config`). Maven `@Parameter` / `@Component` fields are excluded from initializer checks; framework-populated POJOs (`AiPromptDefinition`, `AiModelDefinition`, `AiFieldGenerationConfig`, `AiGenerationConfig`) carry class-level `@SuppressWarnings("NullAway.Init")`. Open follow-up: review remaining unannotated public API surfaces for places where `@Nullable` would be more precise than the implicit non-null default.
